...
/// Handler function for events when the state exists
typedef HandlerFn<Event, State> = FutureOr<State?> Function(Event, State);

/// In some cases it is possible for an [Event] to be received before the first [State] is set.
/// This is the callback for such cases
/// To avoid this, when needed, consider using [waitForState] before adding any [Event]s.
typedef FallbackFn<Event, State> = FutureOr<State?> Function(Event);

/// All types of event handlers
sealed class EventHandler<Event, State> {
  static OptDiffHandler<Event, State> optDiff<Event, State>(
          {required HandlerFn<Event, State> handler, required FallbackFn<Event, State> fallback}) =>
      OptDiffHandler(handler: handler, fallback: fallback);

  static OptSameHandler<Event, State> optSame<Event, State>(HandlerFn<Event, State?> handler) =>
      OptSameHandler(handler);

  static ExistsHandler<Event, State> exists<Event, State>(HandlerFn<Event, State> handler) =>
      ExistsHandler(handler);
}

/// Event handlers for when the state may not exist
sealed class OptEventHandler<Event, State> implements EventHandler<Event, State> {
  static OptDiffHandler<Event, State> diff<Event, State>(
          {required HandlerFn<Event, State> handler, required FallbackFn<Event, State> fallback}) =>
      OptDiffHandler(handler: handler, fallback: fallback);

  static OptSameHandler<Event, State> same<Event, State>(HandlerFn<Event, State?> handler) =>
      OptSameHandler(handler);
}

/// Event handler when the state may not exist and the functionality is different when the state exists vs not.
class OptDiffHandler<Event, State> implements OptEventHandler<Event, State> {
  final HandlerFn<Event, State> handler;
  final FallbackFn<Event, State> fallback;

  OptDiffHandler({required this.handler, required this.fallback});

  FutureOr<State?> handle(Event event, State? state) {
    if (state == null) {
...
/// Event handler when the state may not exist and functionality is the same when the state exists vs not.
class OptSameHandler<Event, State> implements OptEventHandler<Event, State> {
  final HandlerFn<Event, State?> handler;

  OptSameHandler(this.handler);
...
/// Event handler when it is known the state exists.
class ExistsHandler<Event, State> implements EventHandler<Event, State> {
  final HandlerFn<Event, State> handler;

  ExistsHandler(this.handler);
...
class BlocSubject<Event, State> implements BehaviorSubject<State> {
  /// Useful if you want to set logging.
  static FallbackFn? defaultEmptyHandler;
  ...
  BlocSubject._(this._states, EventHandler<Event, State> handler) {
    _eventHandler(handler);
  }

  factory BlocSubject({
    required OptEventHandler<Event, State> handler,
    ...
    return BlocSubject._(behavior, handler);
  }
  ...
  factory BlocSubject.fromBehavior(
    ...
    required OptEventHandler<Event, State> handler,
    FallbackFn<Event, State>? emptyHandler,
  }) {
    return BlocSubject._(behavior, handler);
  }
  ...
  factory BlocSubject.fromValue(
    State val, {
    required ExistsHandler<Event, State> eventHandler,
    void Function()? onListen,
    void Function()? onCancel,
    bool sync = false,
  }) {
  ...
  void _eventHandler(EventHandler<Event, State> handler) {
    if (_transformSubscription != null) {
      _transformSubscription!.cancel();
    }
    final transform = _events.flatMap<State>((event) async* {
      final state = _states.valueOrNull;
      final State? newData;
      switch (handler) {
        case ExistsHandler<Event, State>():
          assert(state is State,
              "Internal logic error, this should never happen. Distinct can only be used provided to the api when it is known to be distinct");
          newData = await handler.handle(event, state as State);
        case OptEventHandler<Event, State>():
          switch (handler) {
            case OptDiffHandler<Event, State>():
              newData = await handler.handle(event, state);
            case OptSameHandler<Event, State>():
              newData = await handler.handle(event, state);
          }
      }
      if (newData == null) {
